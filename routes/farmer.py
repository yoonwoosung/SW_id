# routes/farmer.py — 농장주 간편모드(easy_mode) 라우트 모음 (백 + 프 통합본).
import os
import json
import math
import re
...
def farmer_easy_mode():
    ...
    # AI 후기 요약
    def _build_summary(kw_list, is_strength):
        if not kw_list:
            return None
        top = [kw for kw, _ in kw_list[:3]]
        if len(top) == 1:
            joined = top[0]
        elif len(top) == 2:
            joined = f"{top[0]}와(과) {top[1]}"
        else:
            joined = f"{top[0]}, {top[1]}와(과) {top[2]}"
        if is_strength:
            return f"방문객들이 {joined}을(를) 특히 높이 평가했어요."
        return f"{joined}을(를) 개선하면 더 좋은 평가를 받을 수 있어요."

    feedback_report = {}
    listings_with_reviews = Experience.query.filter(
        Experience.farmer_id == user.id
    ).options(db.joinedload(Experience.reviews)).all()
    for exp in listings_with_reviews:
        if not exp.reviews:
            continue
        strength_kw = defaultdict(int)
        improvement_kw = defaultdict(int)
        pos_count = sum(1 for r in exp.reviews if r.rating >= 4)
        satisfaction_rate = round(pos_count / len(exp.reviews) * 100) if exp.reviews else None
        for review in exp.reviews:
            if review.analysis_result:
                try:
                    data = json.loads(review.analysis_result)
                    for kw in data.get('strengths', []):
                        if kw: strength_kw[kw] += 1
                    for kw in data.get('improvements', []):
                        if kw: improvement_kw[kw] += 1
                except (json.JSONDecodeError, TypeError):
                    continue
        if strength_kw or improvement_kw:
            s_list = sorted(strength_kw.items(), key=lambda x: x[1], reverse=True)
            i_list = sorted(improvement_kw.items(), key=lambda x: x[1], reverse=True)
...
def easy_create_experience():
    ...
    if request.method == 'POST':
        ...
        # [백엔드 전용 기능 유지] 기본 농장 연동
        default_farm = farm_service.default_farm_for(session['user_id'])

        new_experience = Experience(
            farm_id=default_farm.id if default_farm else None, # 백엔드 farm_id 바인딩 유지
