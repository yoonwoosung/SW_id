# routes/farms.py — 농장(Farm) 다중 등록 라우트(관리 페이지 + CRUD API). 얇게 유지, 로직은 farm_service 호출.
import os
import uuid
...
def register(app):
    ...
    app.add_url_rule('/api/farms', 'create_farm', create_farm, methods=['POST'])
    app.add_url_rule('/api/farms/<int:farm_id>', 'update_farm', update_farm, methods=['PUT'])
    app.add_url_rule('/api/farms/<int:farm_id>', 'delete_farm', delete_farm, methods=['DELETE'])
